...
public class MyCassandraTemplate {
	...
	public long getMaxValue(String tableName, String columnName) {
		com.datastax.driver.core.Session ses = cassandraTemplate.getSession();
		
		ResultSet set = ses.execute("select max("+ columnName + ") as " + columnName + " from " + tableName);
		Row row = set.one();
